...
- ✅ パックの追加（ディレクトリ、`.mcpack`、`.mcaddon`、`.zip`）
- ❌ パックの更新
...
| `A` | パスを入力してサーバ全体へパックを追加 |
| `a` | パスを入力してワールド配下へパックを追加 |
| `Esc` / `Ctrl+C` | 終了 |

削除時は、通常はサーバディレクトリ配下の`delpacks/`へパックを移動します。`--force-delete`を指定した場合は、`delpacks/`へ退避せず完全に削除します。

パックの追加では、ディレクトリまたは`.mcpack`、`.mcaddon`、`.zip`ファイルへのパスを入力します。アーカイブ内の複数パックも追加できます。追加先に同種かつ同一UUIDのパック、または同名のディレクトリが存在する場合は、安全のため追加を中止します。
